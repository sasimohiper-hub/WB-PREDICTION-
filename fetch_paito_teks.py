...
async def scrape_with_playwright(slug, browser):
    ...
    page = None
    try:
        # Buat context dengan UA desktop biasa (bukan HeadlessChrome)
        ...
        # Inject JS untuk hide headless fingerprint
        ...
        # Terapkan stealth jika tersedia
        if HAS_STEALTH:
        ...
        # Tunggu semua network request selesai
        await page.wait_for_timeout(10000)

        # Kalau dapat dari API intercept → pakai itu
        if api_data:
            best = max(api_data, key=lambda x: x['count'])
            print(f"  [API] {best['url'][:60]} ({best['count']} angka)")
            return best['text']

        # Coba ambil dari JS variable di memory halaman
        # rumusangkanet kemungkinan simpan data di window.xxx atau var global
        js_data = None
        js_probes = [
            # Coba berbagai nama variable JS yang umum dipakai
            "JSON.stringify(window.paito_data || null)",
            "JSON.stringify(window.data || null)",
            "JSON.stringify(window.result || null)",
            "JSON.stringify(window.results || null)",
            "JSON.stringify(window.angka || null)",
            "JSON.stringify(window.teks || null)",
            # Coba ambil semua variable global yang berisi array angka
            """(() => {
                for (const k of Object.keys(window)) {
                    const v = window[k];
                    if (Array.isArray(v) && v.length > 20) {
                        const s = JSON.stringify(v);
                        if ((s.match(/\\b\\d{4}\\b/g)||[]).length > 20) return s;
                    }
                }
                return null;
            })()""",
            # Coba innerHTML dari elemen yang mungkin berisi data
            "document.querySelector('#paito-content,#data,.paito,.teks-paito,[id*=paito],[class*=paito]')?.innerHTML",
            # Ambil semua text dari script tags (mungkin data inline)
            """(() => {
                const scripts = Array.from(document.querySelectorAll('script'));
                for (const s of scripts) {
                    const t = s.textContent;
                    if ((t.match(/\\b\\d{4}\\b/g)||[]).length > 50) return t.slice(0, 50000);
                }
                return null;
            })()""",
        ]
        ...
        for probe in js_probes:
            try:
                ...
                if result:
                    found = re.findall(r'\b\d{4}\b', str(result))
                    if len(found) >= 20:
                        print(f"  [JS VAR] probe hit → {len(found)} angka")
                        js_data = str(result)
                        break
            except Exception:
                continue
        ...
        # DEBUG: dump semua URL yang diakses
        import os as _os
        ...
        if _os.environ.get('DEBUG_HTML') or '--test' in __import__('sys').argv:
            ...
            print(f"  [DEBUG] Body: {body_txt!r}")
            # Log semua script src dan inline script sizes
            scripts_info = await page.evaluate("""() => {
                const ss = Array.from(document.querySelectorAll('script'));
                return ss.map(s => ({
                    src: s.src || '',
                    len: s.textContent.length,
                    has4d: (s.textContent.match(/\\b\\d{4}\\b/g)||[]).length
                }));
            }""")
            print(f"  [DEBUG] Scripts: {scripts_info[:5]}")
