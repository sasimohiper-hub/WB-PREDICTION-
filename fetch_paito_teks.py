...
async def scrape_with_playwright(slug, browser):
    """Fetch 1 halaman pakai Playwright browser instance yang sudah ada."""
    ...
    try:
        ...
        # Halaman pakai JS lazy load — "Memuat data..." dulu baru angka muncul
        # Tunggu sampai elemen dengan angka 4 digit muncul di DOM
        # Coba beberapa selector yang mungkin dipakai rumusangkanet
        data_loaded = False
        for selector in [
            'pre',                          # <pre>1234 5678...</pre>
            'td',                           # tabel
            '.paito-content',               # class umum
            '.result',
            '#paito-data',
            'div[class*="paito"]',
            'div[class*="result"]',
        ...
        # Tunggu JS selesai render data (max 15 detik)
        try:
        ...
        if _os.environ.get('DEBUG_HTML') or '--test' in __import__('sys').argv:
            debug_path = f"debug_{slug}.html"
            with open(debug_path, 'w', encoding='utf-8') as df:
                df.write(html)
            from bs4 import BeautifulSoup as _BS
            _soup = _BS(html, 'html.parser')
            _body = _soup.get_text(separator=' ', strip=True)
            # Cari angka 4 digit di body
            import re as _re
            digits_found = _re.findall(r'\b\d{4}\b', _body)
            print(f"\n  [DEBUG] HTML: {len(html)} chars | 4-digit: {len(digits_found)} found")
            if digits_found:
                print(f"  [DEBUG] Sample digits: {digits_found[:10]}")
            else:
                print(f"  [DEBUG] Body sample: {_body[:300]}")
        return html
