...
async def scrape_with_playwright(slug, browser):
    ...
    try:
        ...
        # Tunggu sampai ada angka 4 digit di halaman
        try:
            await page.wait_for_function(
                "() => document.body.innerText.match(/\\b\\d{4}\\b/)",
                timeout=15000
            )
        except Exception:
            pass
        html = await page.content()
        # DEBUG: simpan sample HTML ke file (hanya saat test)
        import os
        if os.environ.get('DEBUG_HTML') or '--test' in __import__('sys').argv:
            debug_path = f"debug_{slug}.html"
            with open(debug_path, 'w', encoding='utf-8') as df:
                df.write(html)
            # Print 500 char pertama body text untuk diagnosis
            from bs4 import BeautifulSoup as _BS
            _soup = _BS(html, 'html.parser')
            _body = _soup.get_text(separator=' ', strip=True)
            print(f"\n  [DEBUG] body text sample (500c): {_body[:500]}")
