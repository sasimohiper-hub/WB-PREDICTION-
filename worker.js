...
// Membuat tampilan mirip screenshot: history putaran + prediksi
function formatRumus(item, resultMap) {
  // resultMap: { [wbId]: [ { result, periods } ] } — data result dari KV (opsional)
  // Jika tidak ada data result, tampilkan prediksi saja

  const wbLabel = getWbLabel(item.wbId);
  const jenisLabel = item.jenis === 'ai_ke' ? 'AI KE' :
                     item.jenis === 'bbfs_ke' ? 'BBFS KE' :
  ...
  text += `HIT   : ${item.limit}   PATAH: ${item.maxPatah}x\n`;
  text += `\`\`\`\n\n`;

  // Prediksi sekarang
  text += `🎯 *${jenisLabel} : ${item.predNow}*\n`;
  text += `${'═'.repeat(16)}\n`;
  text += `Acc ${item.acc} | Limit ${item.limit} putaran | patah ${item.maxPatah}x`;
...
async function cmdPrediksi(chatId, env) {
  try {
    ...
    // Kelompokkan per pasaran
    const byWb = {};
    for (const item of items) {
      if (!byWb[item.wbId]) byWb[item.wbId] = [];
      byWb[item.wbId].push(item);
    }

    // Kirim per pasaran
    for (const wbId of Object.keys(byWb)) {
      const rumusList = byWb[wbId];
      let text = `${getWbLabel(wbId)}\n`;
      text += `📅 Update: ${new Date(bankData.exportedAt).toLocaleString('id-ID')}\n\n`;

      for (const item of rumusList) {
        const jenisLabel = item.jenis === 'ai_ke' ? 'AI KE' :
                           item.jenis === 'bbfs_ke' ? 'BBFS KE' :
                           item.jenis.toUpperCase();
        text += `┌─ \`${item.formula}\`\n`;
        text += `│  Rumus : ${jenisLabel}\n`;
        text += `│  Limit : ${item.limit} | Patah: ${item.maxPatah}x\n`;
        text += `└─ 🎯 *${jenisLabel}: ${item.predNow}*\n\n`;
      }
    ...
    for (const member of members) {
      try {
        ...
        for (const wbId of Object.keys(byWb)) {
          ...
          await sendMsg(member.id, text, { parse_mode: 'Markdown' });
          await sleep(100);
        }
